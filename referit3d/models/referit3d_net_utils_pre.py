...
def single_epoch_train(model, data_loader, criteria, optimizer, device, pad_idx, args, tokenizer=None,epoch=None):
    ...
    for batch in tqdm.tqdm(data_loader):
        ...
        order = []
        B = len(batch['referential_order'][0])
        for i in range(B): # batch size
            for j in range(args.order_len):
                order.append(batch['referential_order'][j][i])
        
        order_tokens = tokenizer(order, return_tensors='pt', padding=True)

        for name in order_tokens.data:
            order_tokens.data[name] = order_tokens.data[name].cuda()

        batch['order_tokens'] = order_tokens
        batch['pred_class_mask'] = batch['pred_class_mask'].cuda()
        batch['order_labels'] = batch['order_labels'].cuda()
        if args.lang_multilabel:
            batch['anchor_ind'] = batch['anchor_ind'].cuda()
        if args.multilabel_pretraining:
            batch['ordered_multilabel_gt'] = batch['ordered_multilabel_gt'].cuda()
            batch['rel_coors'] = batch['rel_coors'].cuda()
            batch['center_coors'] = batch['center_coors'].cuda()
            batch['corner_coors'] = batch['corner_coors'].cuda()
            batch['obj_mask'] = batch['obj_mask'].cuda().squeeze()
...
def evaluate_on_dataset(model, data_loader, criteria, device, pad_idx, args, randomize=False, tokenizer=None):
    ...
    for batch in tqdm.tqdm(data_loader):
        ...
        order = []
        B = len(batch['referential_order'][0]) # batch size
        for i in range(B):
            for j in range(args.order_len):
                order.append(batch['referential_order'][j][i])
